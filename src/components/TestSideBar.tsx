...
export const TestSideBar = () => {
  const { videos, setVideos, toggleFullScreen, removeVideo, updateVideo } =
    useVideo();
  ...
  const togglePlayer = (video: Video, idx: number) =>
    updateVideo("playing", !video.playing, idx);
    ...
    <>
      ...
      >
        ...
        {showButtons && (
          <ButtonContainer>
            <StyledSideButton
              onClick={() => setVideos([{ ...sampleVid, url: oneVid }])}
            >
              1
            </StyledSideButton>
            <StyledSideButton
              onClick={() =>
                setVideos(twoVid.map((item) => ({ ...sampleVid, url: item })))
              }
            >
              2
            </StyledSideButton>
            <StyledSideButton
              onClick={() =>
                setVideos(threeVid.map((item) => ({ ...sampleVid, url: item })))
              }
            >
              3
            </StyledSideButton>
            <StyledSideButton
              onClick={() =>
                setVideos(fourVid.map((item) => ({ ...sampleVid, url: item })))
              }
            >
              4
            </StyledSideButton>
        ...
        {videos.length !== 0 && (
          <StyledSideButton onClick={() => toggleFullScreen()}>
            Full Screen
          </StyledSideButton>
        )}
        <VideoInputModal />
