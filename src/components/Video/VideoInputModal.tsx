...
import { VideoInfo } from '../../typings/Video';
...
const VideoInputModal = () => {
  ...
  const processYouTubeVideo = async (url: string) => {
    ...
    if (videoInfo) {
      ...
      addVideo({ ...sampleVid, url, name: videoTitle });
    }
  };

  const isYouTubeVideo = (url: string) => url.includes('www.youtube.com/watch?v=');
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (isYouTubeVideo(inputValue)) {
        ...
        addVideo({ ...sampleVid, url: inputValue, name: inputValue });
      }
